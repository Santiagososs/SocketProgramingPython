...
def post_it(salvar_cache, imag_str):
    ...
    if salvar_cache.find(b'html') and imag_str != 'jpg':
        respostaEmString = codecs.decode(salvar_cache, encoding='utf-8')
...
def conexao_browser(str_dominio, url_Complexa_Divisao, urlTratamento, conexao, addr,imag_str):
    ...
    carregamento_pag_com_post_it = post_it(carregamento_pag, imag_str)
    conexao.sendall(carregamento_pag_com_post_it.encode(errors='ignore'))
...
def main():
    while True:
        ...
        if(str_dominio == 'favicon.ico' and imag_str != 'jpg'):
            continue
        else:
            try:
